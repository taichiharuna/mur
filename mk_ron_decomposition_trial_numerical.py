...
for n in [200,400,800]:
    for i in range(M):
        ...
        for T in [5000,10000,20000]:
            ...
            f = open("ron_ev_numerical_a{:.2f}_d{:.3f}_n{:d}_t{:d}_tr{:d}.txt".format(alpha,delta,n,T,i),"w")
            for j in range(n):
                f.write("{:.15f}\n".format(a[n-1-j]))
            f.close()
            ...
            f = open("ron_mk_numerical_a{:.2f}_d{:.3f}_n{:d}_t{:d}_tr{:d}.txt".format(alpha,delta,n,T,i),"w")
            for j in range(kmax):
                f.write("{:d} {:.15f} {:.15f}\n".format(j,mk[j],mk0[j]))
            f.close()
        ...
        f = open("ron_ev_theoretical_a{:.2f}_d{:.3f}_n{:d}_tr{:d}.txt".format(alpha,delta,n,i),"w")
        for j in range(n):
            f.write("{:.15f}\n".format(a[n-1-j]))
        f.close()
        ...
        f = open("ron_mk_theoretical_a{:.2f}_d{:.3f}_n{:d}_tr{:d}.txt".format(alpha,delta,n,i),"w")
        for j in range(kmax):
            f.write("{:d} {:.15f} {:.15f}\n".format(j,mk[j],mk0[j]))
        f.close()
